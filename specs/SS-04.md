...
class SecureDuckDBEngine:
    ...
    def load_product_data(self, product: DataProduct, schema: DataSchema):
        ...
        # SECURITY: sanitize identifier to prevent SQL injection
        safe_id = re.sub(r'[^a-zA-Z0-9_]', '_', product.product_id)
        self.db.register(safe_id, table)

        # 3. 自动创建脱敏视图（高敏感字段自动替换）
        self._create_masked_view(safe_id, schema)
...
class DataModelingRuntime:
    ...
    def execute_modeling_task(self, script: str, session_id: str):
        ...
        """
        # NOTE: exec() on untrusted code is inherently dangerous even with restricted builtins.
        # The recommended approach is to run user code in a subprocess with seccomp+bwrap isolation
        # (see SS-03 CodeScanner for AST pre-scan, and bwrap sandbox for runtime isolation).
        # The following is ONLY for pre-scanned, trusted code in controlled environments.
        import ast as _ast
        tree = _ast.parse(script)  # Syntax check only; CodeScanner should have already validated
        sandbox_env = {
            "db": self.db,
            "internal_storage": self.internal_storage,
            "__builtins__": RESTRICTED_BUILTINS,  # 移除 open/exec/eval 等
        }
        # SECURITY: Code MUST pass CodeScanner AST analysis before reaching this point
        exec(compile(tree, "<sandbox>", "exec"), sandbox_env)
```
