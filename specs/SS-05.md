...
class InspectionRuleRegistry:
    RULE_SETS: dict[SandboxMode, list[InspectionStage]] = {
        ...
        SandboxMode.UNSTRUCTURED_LLM_SFT: [
            ...
            TextMemorizationStage(threshold=0.08),  # 检测模型是否记忆训练文本
            WatermarkStage(method="model_weight_shift"),
            SignAndAuditStage(),
        ],
        SandboxMode.UNSTRUCTURED_VISION: [
            FormatCheckStage(allowed_formats=["safetensors", "onnx", "json_metrics", "json_eval"]),
            NoRawImageStage(),                 # 绝对禁止任何图像输出
            ModelWeightSafetyStage(),
            ImageReconstructionProbeStage(),   # 通过模型反推原图检测
            WatermarkStage(method="model_weight_shift"),
            SignAndAuditStage(),
        ],
        SandboxMode.UNSTRUCTURED_MULTIMODAL: [
            FormatCheckStage(allowed_formats=["safetensors", "json_metrics"]),
            NoRawImageStage(),
            NoRawAudioStage(),
            MIAProbeStage(threshold=0.12, model_type="multimodal"),
            ImageReconstructionProbeStage(),
            WatermarkStage(method="model_weight_shift"),
            SignAndAuditStage(),
        ],
        SandboxMode.SEMI_STRUCTURED_ETL: [
            FormatCheckStage(allowed_formats=["json", "csv_aggregate"]),
            PIIDetectionStage(block_on_hit=True),
            KAnonymityStage(k_min=5),
            DifferentialPrivacyStage(mechanism="laplace"),
            WatermarkStage(method="zero_width"),
...
class PIIDetectionStage(InspectionStage):
    ...
    REGEX_PATTERNS = {
        ...
        "name_zh":    r"(?:先生|女士|同志|老师|教授|医生|主任|经理|总|董)[\u4e00-\u9fa5]{1,3}",  # 中文姓名（仅匹配带称谓，避免误报）
    }
...
class MIAProbeStage(InspectionStage):
    ...
    方法：Shadow Model Attack（Shokri et al.）的简化版
    """
    ...
    def inspect(self, output: Any, context: InspectionContext) -> StageResult:
        ...
        member_samples = context.dataloader.get_member_samples(n=200)
        nonmember_samples = context.dataloader.get_holdout_samples(n=200)
